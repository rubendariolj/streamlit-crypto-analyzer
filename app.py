...
st.set_page_config(layout='wide', page_title='Streamlit Crypto TA Analyzer — Robust v2.3')
st.title('Streamlit Crypto TA Analyzer — Robust v2.3')

# ------------------------------------------------------------
# 🔍 Kaleido availability check (for ALL users / environments)
# ------------------------------------------------------------
def check_kaleido():
    try:
        _ = pio.to_image  # attribute exists when plotly can call kaleido
        # Try a no-op export to truly verify engine availability
        try:
            # Create a tiny figure to validate
            test_fig = go.Figure(data=[go.Scatter(x=[0,1], y=[0,1])])
            _ = pio.to_image(test_fig, format="png", engine="kaleido", scale=1)
            return True
        except Exception:
            return False
    except Exception:
...
if not st.session_state["kaleido_ok"]:
    st.warning("⚠️ Kaleido is not available. Chart images will NOT be embedded in PDFs until it's installed in this environment.")
    ...
    with col1:
        if st.button("📋 Copy Install Command"):
            try:
                ...
                st.warning("Clipboard not accessible from this browser/session. Please copy manually.")
    with col2:
        if st.button("🔄 Recheck Kaleido"):
            ...
            if st.session_state["kaleido_ok"]:
                ...
                st.error("❌ Still not found. Ensure it's installed in the same Python environment running this app.")
else:
    st.success("✅ Kaleido available — PDF chart export with embedded chart is enabled.")

# Force Kaleido defaults (helps on Streamlit Cloud)
try:
...
def add_chart_to_pdf(story, fig, styles):
    if fig is not None:
        try:
            ...
            story.append(Paragraph("⚠️ Chart image could not be embedded (Kaleido missing or failed).", styles["Italic"]))
            story.append(Spacer(1, 12))
    else:
        story.append(Paragraph("⚠️ No chart available to embed.", styles["Italic"]))
        story.append(Spacer(1, 12))
...
def build_pdf_bytes(token_text, timeframe, df, st_retr, lt_retr, scenario_df, fig=None):
    ...
    # Embed chart image
    add_chart_to_pdf(story, fig, styles)
